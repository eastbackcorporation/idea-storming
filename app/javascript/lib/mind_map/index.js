...
import {
  ...
  d3PanZoom,
  onTick
} from '../common/utils/d3'

import { getDimensions, getViewBox } from '../common/utils/dimensions'
import nodeToHTML from '../common/utils/nodeToHTML'
...
export default {
  ...
  methods: {
    ...
    /**
     * update mind map to JSON
     */
    updateMap () {
      // this.connections.forEach(connection => console.log(connection.source, connection.target, connection.curve))
      // this.nodes.forEach(node => console.log(node.fx, node.fy))

      const json = {
        connections: this.connections,
        nodes: this.nodes
      }
      this.$emit('update-event', json)
    },
    ...
    renderMap () {
      ...
      var self = this
      svg.selectAll('.addIdea').on("click", function(d, i) {
        var id = $(this).data('node-id')
        self.$emit('add-node-event', id)
      });
    }
  },
  mounted () {
  ...
  render () {
    return (
      <div>
        <svg class='mindmap-svg' ref='mountPoint' style='border: 1px solid gray;'/>
        <div class='fixed-bottom'>
          <div class="m-3 clearfix">
            <button class='btn btn-success float-right' onClick={this.updateMap}>保存</button>
          </div>
        </div>
      </div>
    )
